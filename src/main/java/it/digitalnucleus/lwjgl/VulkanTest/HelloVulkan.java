...
import static it.digitalnucleus.lwjgl.VulkanTest.VKUtil.*;
import static org.lwjgl.glfw.GLFW.*;
import static org.lwjgl.glfw.GLFWVulkan.*;
import static it.digitalnucleus.lwjgl.VulkanTest.DemoUtils.*;
...
public class HelloVulkan {
    ...
    private static long createPipeline(VkDevice device, long renderPass, VkPipelineVertexInputStateCreateInfo vi) throws IOException {
        ...
        shaderStages.get(0).set(loadShader(device, "it/digitalnucleus/lwjgl/VulkanTest/triangle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT));
        shaderStages.get(1).set(loadShader(device, "it/digitalnucleus/lwjgl/VulkanTest/triangle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT));
    ...
    public static void main(String[] args) throws IOException {
        ...
        GLFWWindowSizeCallback windowSizeCallback = new GLFWWindowSizeCallback() {
            public void invoke(long window, int width, int height) {
                if (width <= 0 || height <= 0)
                    return;
                HelloVulkan.width = width;
                HelloVulkan.height = height;
